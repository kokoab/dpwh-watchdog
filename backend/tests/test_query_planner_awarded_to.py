...
try:
    import features.chat.agent.query_planner as query_planner
...
def _fallback_plan(query: str):
    with (
        patch("features.chat.agent.query_planner.get_entity_catalog", return_value=_catalog()),
...
class QueryPlannerAwardedToTests(unittest.TestCase):
    ...
    def test_last_five_years_window_uses_current_year_inclusively(self) -> None:
        with patch("features.chat.agent.query_planner._current_year", return_value=2026):
